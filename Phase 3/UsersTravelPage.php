    ...
    ?>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const deleteLinks = document.querySelectorAll('.editLinks[href*="deleteTravel.php"]');

                deleteLinks.forEach(link => {
                    link.addEventListener('click', function (event) {
                        event.preventDefault(); // Prevent the link from working normally

                        if (confirm("Are you sure you want to delete this travel?")) {
                            const travelID = new URL(this.href).searchParams.get("travel_id");

                            // Simple AJAX using XMLHttpRequest
                            const xhr = new XMLHttpRequest();
                            xhr.open("GET", `deleteTravel.php?travel_id=${encodeURIComponent(travelID)}`, true);
                            xhr.onload = function () {
                                if (xhr.status === 200) {
